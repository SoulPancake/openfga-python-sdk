...
async def create_store(openfga: OpenFgaClient) -> str:
    ...
    """
    response = await openfga.create_store(
...
async def demonstrate_duplicate_writes_error(openfga: OpenFgaClient):
    ...
    print(
        f"  Successfully wrote: {tuple_to_write.user} is {tuple_to_write.relation} of {tuple_to_write.object}"
    )
    ...
    try:
        ...
        print("  This is the default behavior when writing duplicate tuples.")
...
async def demonstrate_missing_deletes_error(openfga: OpenFgaClient):
    ...
    try:
        ...
        print(f"  Expected error occurred: {type(e).__name__}")
        print("  This is the default behavior when deleting non-existent tuples.")
...
async def demonstrate_missing_deletes_ignore(openfga: OpenFgaClient):
    ...
    print(
        "  Success! The delete of non-existent tuple was silently ignored (no error thrown)."
    )
...
async def demonstrate_combined_conflict_options(openfga: OpenFgaClient):
    ...
    print(
        f"  - Writing (may be duplicate): {write_tuple.user} is {write_tuple.relation} of {write_tuple.object}"
    )
    print(
        f"  - Deleting (may not exist): {delete_tuple.user} is {delete_tuple.relation} of {delete_tuple.object}"
    )

    # First write
    await openfga.write(
        ClientWriteRequest(writes=[write_tuple], deletes=[delete_tuple]), options
    )
...
async def demonstrate_explicit_error_mode(openfga: OpenFgaClient):
    ...
    try:
        ...
        print(f"  Expected error occurred: {type(e).__name__}")
        print("  This demonstrates that ERROR mode is the default behavior.")
...
async def main():
    ...
    async with OpenFgaClient(configure) as openfga:
        ...
        try:
            ...
            print_checkpoint("Summary", "All demonstrations completed successfully!")

            print("Key Takeaways:")
            print(
                "  1. on_duplicate controls behavior when writing tuples that already exist"
            )
            print("     - ERROR (default): Returns an error")
            print("     - IGNORE: Treats as no-op, no error thrown")
            print("")
            print(
                "  2. on_missing controls behavior when deleting tuples that don't exist"
            )
            print("     - ERROR (default): Returns an error")
            print("     - IGNORE: Treats as no-op, no error thrown")
            print("")
